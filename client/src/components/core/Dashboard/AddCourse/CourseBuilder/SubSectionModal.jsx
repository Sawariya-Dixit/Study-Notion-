...
export default function SubSectionModal({
  ...
  const isFormUpdated = () => {
    ...
    const titleChanged =
      v.lectureTitle?.trim() !== modalData?.title?.trim()
    const descChanged =
      v.lectureDesc?.trim() !== modalData?.description?.trim()
  ...
  const handleEditSubsection = async () => {
    try {
      ...
      const updatedSub = normalizeResult(result)
      ...
      if (payload?.SubSection) {
        const updated = course.courseContent.map((section) =>
          section._id === modalData.sectionId
            ? { ...section, SubSection: payload.SubSection }
            : section
        )
        ...
        const updated = course.courseContent.map((section) =>
          section._id === modalData.sectionId
            ? {
                ...section,
                SubSection: [...(section.SubSection || []), payload],
              }
            : section
        )
        dispatch(setCourse({ ...course, courseContent: updated }))
  ...
  return (
    <div className="fixed inset-0 z-[1000] grid place-items-center bg-black/40 backdrop-blur-sm">
      <div
        className="
          w-11/12 max-w-[700px] max-h-[90vh]
          rounded-lg
          bg-[var(--richblack-800)]
          border border-[var(--richblack-400)]
          flex flex-col
        "
      >
        {/* HEADER */}
        <div className="flex items-center justify-between bg-[var(--richblack-700)] p-4 sm:p-5">
          ...
          <RxCross2
            ...
            className="cursor-pointer text-2xl text-[var(--richblack-5)]"
          />
        </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="flex flex-col gap-2">
              <label className="text-sm text-[var(--richblack-5)]">
                Lecture Title{" "}
                {!view && <sup className="text-[var(--pink-200)]">*</sup>}
              </label>
              <input
                disabled={view}
            ...
            <div className="flex flex-col gap-2">
              ...
              <textarea
                ...
                className="
                  min-h-[120px] rounded-md px-3 py-2
                  bg-[var(--richblack-700)]
                  border border-[var(--richblack-400)]
                  text-[var(--richblack-5)]
                "
