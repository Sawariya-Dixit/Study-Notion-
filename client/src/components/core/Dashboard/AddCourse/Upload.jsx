...
export default function Upload({
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    accept: accept
      ? accept
      : !video
      ? { "image/*": [".jpeg", ".jpg", ".png"] } // only images
      : { "video/*": [".mp4", ".mov", ".mkv"] }, // only videos
    onDrop,
    multiple: false,
  })
        ...
        {previewSource ? (
          <div className="flex w-full flex-col p-6">
            {!video ? (
              ...
              <video src={previewSource} className="w-full max-h-[300px] rounded-md" controls />
            )}
            {!viewData && (
              <button
                type="button"
